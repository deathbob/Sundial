...
#                              2453097.01224153
#    (current_julian_date + (0.0053 * Math.sin(solar_mean_anomaly.degrees)) - (0.0069 * Math.sin(2 * ecliptic_longitude))).round(4)
     (approximate_solar_noon + (0.0053 * Math.sin(solar_mean_anomaly.degrees)) - (0.0069 * Math.sin(2 * ecliptic_longitude))).round(4)
  end

  def declination_of_the_sun # δsun
    # returns in degrees 4.7585
    (Math.asin(Math.sin(ecliptic_longitude.degrees) * Math.sin(23.45.degrees)).radians).round(4)
  end
